...
function QuizManageContent({
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setLesson({ id: lessonData.id, documentId: lessonData.documentId, Title: lessonData.Title });
  ...
  return (
    <main className="min-h-screen text-slate-100 pb-16 px-4 sm:px-6 lg:px-8">
      <div className="max-w-3xl mx-auto space-y-8">
        <BackButton href={`/dashboard/courses/${courseId}/lessons/${lessonId}`} label="Back to Lesson" />

        <div>
          <h1 className="text-2xl font-bold text-white">Quiz: {lesson.Title}</h1>
          <p className="text-sm text-slate-400 mt-1">
            Add, edit, or remove the MCQ questions students will see when they take this lesson's quiz.
          </p>
        </div>
        ...
        <div className="space-y-4">
          <h2 className="text-xl font-bold text-white">Quiz Questions ({quizzes.length})</h2>
