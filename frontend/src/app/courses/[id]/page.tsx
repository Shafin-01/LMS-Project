...
interface Lesson {
  id: number;
  documentId?: string; // এই লাইনটি যোগ করতে হবে
  Title: string;
}
...
export default async function CourseDetailPage({
    ...
    try {
        const response = await fetchAPI<CourseResponse>(
            `/courses/${courseId}?populate=*`
        );
                    ...
                    {(!course.lessons || course.lessons.length === 0) ? (
                        ...
                        <div className="space-y-3">
                            {course.lessons.map((lesson, index) => (
                                <Link
                                    key={lesson.id}
                                    href={`/courses/${course.documentId || course.id}/lessons/${lesson.documentId || lesson.id}`}
